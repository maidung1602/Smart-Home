    ...
    <body class="sb-nav-fixed">
        ...
        <div id="layoutSidenav" style="display: flex; background-color: #eee; height: 736px">
            ...
            <div style=" margin: 178px 0 50px; background-color: white; padding: 56px 100px 0;border-radius: 30px;height: 354px;" class="col-sm-5">
                <form action="admin-edit-productmodel" id="add-form" method="POST">
                    <%-- Product Model Forms --%>
                    <section id="product-models-wrapper">
                        <section class="product-models">
                            <input type="hidden" name="id" value="${productmodel.id}" />
                            <input type="hidden" name="product-id" value="${productmodel.productId}" />
                            <div>
                                <label for="variant">Model name: </label>
                                <input style="margin-left: 6px; width: 260px" value="${productmodel.variant}"  name="variant" type="text" id="variant">
                            </div>
                            <section style="display: flex">
                                <div style="text-align: center;">
                                    <label for="origin-price">Origin Price: </label>
                                    <input style="width: 80px; " name="origin-price" value="${productmodel.origin_price}" type="number" id="origin-price">
                                </div>
                                <div style="text-align: center;">
                                    <label for="sale-price">Sale price: </label>
                                    <input style="width: 80px; " name="sale-price" value="${productmodel.sale_price}" type="number" id="sale-price">
                                </div>
                                <div style="text-align: center;">
                                    <label for="quantity">Quantity: </label>
                                    <input style="width: 80px; " name="quantity" value="${productmodel.quantity}" type="number" id="quantity">
                                </div>
                            </section>
                            <div>
                                <label for="image">Image URL: </label>
                                <input style="margin-left:8px; width: 334px" value="${productmodel.image}" name="image" type="text" id="image">
                            </div>
                        </section>
                    </section>
                    <hr style="color: #000 !important" style="width: 40px !important">
                    <div>
                        <button id="add-product" style="background-color: #0abbcb; margin:4px 160px 16px" value="Add product" onclick="filterEmpty()">Save</button>
                    </div>
                </form>
            </div>
        </div>     
